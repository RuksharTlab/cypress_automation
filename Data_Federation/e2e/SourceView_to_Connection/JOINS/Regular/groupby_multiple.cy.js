...
describe('Federation View Automation with Multiple Group By Columns', () => {
  it('Creates a federated view with multiple group by columns and verifies the output', () => {
    // Load test data from fixture
    cy.fixture('test-data/groupby_multiple.json').then((testData) => {
          ...
          cy.get('tbody tr').each(($row, index) => {
            cy.wrap($row).within(() => {
              ...
              cy.get('td').then(($cells) => {
                const values = $cells.map((_, el) => el.textContent.trim()).get();
                
                // Log the row values for debugging
                cy.log(`Row ${index + 1} values:`, values);
                
                // Verify that the values are not empty
                values.forEach((value, i) => {
                  expect(value, `Cell ${i + 1} in row ${index + 1} should not be empty`).to.not.be.empty;
                });
              });
            });
          });
        });
        cy.screenshot('groupby_multiple_end');
    });
  });
});
