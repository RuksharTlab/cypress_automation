...
describe('Federation View Automation with Nested Filters in Both Groups', () => {
  ...
  it('Creates a federated view with nested filters in both groups and verifies the output', () => {
    // Load test data from fixture
    cy.fixture('test-data/nested_and_nested_filter.json').then((testData) => {
      ...
      // 8. Add second group and apply filters with nested condition
      cy.contains('button', '+Add New Group').click();
      applyFilterGroup('Group 2', secondGroupFilter);
      applyNestedFilter('Group 2', secondGroupNestedFilter);

      cy.screenshot("nested_and_nested_filters")
      // 9. Apply the filters
      ...
      cy.screenshot('nested_and_nested_filter_end');
    });
  });
});
