...
let testData;
before(() => {
  cy.fixture('test-data/groupby_complex.json').then((data) => {
    testData = data;
  });
});
...
describe('Federation View Automation', () => {
  it('Performs federation view actions', () => {
    ...
    cy.get('#editor').click();
    const groupByString = `${testData.keyword} ${testData.connectionName}_${testData.schemaName}_${testData.tableName}.${testData.groupbycolumn2}`;
    ...
    cy.get('div.bg-white.shadow.overflow-hidden.h-\\[32\\%\\].z-50', { timeout: 15000 })
      ...
      cy.screenshot('groupby_complex_end');
  });
});